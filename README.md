...
The opening is The Artifact: sixteen beveled structural leaves form one graphite object. Recessed apertures reveal real interface material inside its closed shell. Scroll progressively releases its structural leaves. Pointer response, inertial drag, keyboard controls and direct layer selection expose its construction. Scrolling opens a passage and moves the camera through it into the light origin chapter. A CSS composition remains available without WebGL.
...
Every app has seven states from its source interface through activation, decomposition, analysis, transformation, resolution and its own closure. Native scrolling controls the sequences; optional replay runs for 30–34.4 seconds. A second keyboard-accessible range selects a task-specific inspection region. Diagrams illustrate relationships, not measured results or cross-application integration.
...
- `js/narrative.js`: finite record accumulation, alignment and method formation.
