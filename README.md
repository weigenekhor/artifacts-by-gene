...
Full captures are displayed in the opening and collection. Smaller source details accompany the conceptual demonstrations. Every app opens a fitted original with an actual-size option. The original-resolution limit remains: these sources are not native 4K screenshots. The archive assets under `assets/artifacts/apps/` are preserved but not used by the exhibition.
