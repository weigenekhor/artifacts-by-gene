  ...
  return {
    ...
    render(time, p, px, py, reduced, dt, state) {
      const progress = reduced ? 0 : p,
        opening = Math.max(state.spread, ease((progress - 0.12) / 0.55)),
        exit = ease((progress - 0.72) / 0.28);
      canvas.inert = !reduced && progress > 0.72;
      const yaw = -0.55 + state.yaw * 0.35 + (reduced ? 0 : px * 0.07),
      ...
      const mobile = width < 650,
        scale =
          ...
          (1 - exit * 0.16),
        cx = width * (mobile ? 0.5 : 0.65) + exit * width * 0.85,
        cy = height * (mobile ? 0.57 : 0.61);
      const sy = Math.sin(yaw),
        co = Math.cos(yaw),
        sp = Math.sin(pitch),
        cp = Math.cos(pitch);
      const project = (v, i, down = 0) => {
        const x = v[0] + (i - 7.5) * opening * 0.044,
          y = v[1],
